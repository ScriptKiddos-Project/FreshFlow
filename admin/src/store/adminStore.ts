...
// Updated AdminState interface to match the expected properties
interface AdminState {
  ...
  clearAuth: () => void;
  logout: () => void; // Keep existing logout for compatibility
}
...
export const useAdminStore = create<AdminState>((set) => ({
  ...
  setRole: (role: AdminRole) => set({ role }),
  ...
  // Keep existing logout method for backward compatibility
  logout: () => set({ 
    user: null, 
    isAuthenticated: false, 
    permissions: [], 
...
// Chart data structure interfaces (keeping existing chart interfaces)
